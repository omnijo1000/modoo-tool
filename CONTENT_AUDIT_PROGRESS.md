...
## ⚠️ "완료" 표시와 실제 상태 불일치 재발견 (2026-08-04)

애드센스 재승인 신청 직후 재검증 목적으로 `class="seo"` 블록 길이 재스캔(main 브랜치, 319개 파일 대상)한 결과 **175개 파일이 1500자 미만**으로 나옴 — 위 "5차 스캔·전수검증까지 전부 완료" 기록과 모순. 원인 추정: 그 "완료" 표시들은 시점별로 "그때 스캔 대상에 걸린 파일"만 커버했고, 이후 배치(예: 배치 26 "신규 툴 추가")에서 만들어진 파일이나 스캔 사각지대에 있던 파일은애초에 재검증 대상에서 빠져있었던 것으로 보임. **앞으로 "완료"라고 적혀 있어도 재스캔으로 실제 길이 재확인할 것 — grep/스캔 없이 문서 기록만 믿지 말 것.**
