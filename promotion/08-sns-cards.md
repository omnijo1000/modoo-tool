...
**플랫폼별 게시 방식**: Instagram·Threads는 카드 이미지(캐러셀) 첨부, **X는 텍스트 원고만 게시(이미지 첨부 안 함)** — X 피드 미리보기는 1:1~2:1 비율만 안 잘리고 4:5 카드는 약 16:9로 센터크롭되어 헤드라인/CTA가 잘릴 수 있음, X는 원래 텍스트 훅이 잘 먹히는 플랫폼이라 크롭 대응용 별도 이미지 안 만들고 텍스트로만 감.
